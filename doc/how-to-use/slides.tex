...
\begin{frame}%[fragile]
	...
	\begin{columns}
		\column{.5\textwidth}
			%\only<1>{\includegraphics[scale=.5]{hw_ce}}
			%\only<2>{\includegraphics[scale=.5]{hw_ce}}
			%\only<3>{\includegraphics[scale=.5]{hw_ce}}
			%\only<4>{\includegraphics[scale=.5]{hw_ce}}
		\column{.5\textwidth}
			\begin{itemize}
				\item Manual selection and elimination of features
				\item Automatic selection and elimination of features (based on current manual decisions)
				\item Determining if current configuration is valid (assumption: unspecified features are eliminated)
				\item Counting number of possible configurations
			\end{itemize}
			On save:
			\begin{itemize}
...
\begin{frame}%[fragile]
	...
	\begin{columns}
		...
		\column{.5\textwidth}
			\begin{itemize}
				\item Syntax-Highlighting for Jak files
				\item Content assist (simple version)
				\item Outline view
			\end{itemize}
			On save:
			\begin{itemize}
...
\begin{frame}%[fragile]
	...
	\begin{columns}
		...
		\column{.5\textwidth}
			\begin{itemize}
				\item Visualizes roles that features add to classes
