...
public abstract class SatProblemTests {
	...
	public void testAddFormulasCollection() {
		final SatProblem instance =
			getInstance();
		final Collection<Node> in =
			Arrays.<Node> asList(new Literal("A", false), new Or("A", "B"));
		instance.addFormulas(in);
		final List<Node> expected =
			new LinkedList<>();
		expected.add(new Or(new Literal("A", false)));
	...
	public void testGetClause() {
		...
		final Node expected =
			new Or(new Literal("B", false), "A");
		final Node actual =
			instance.getClause(2);
		assertEquals(expected, actual);
	}
