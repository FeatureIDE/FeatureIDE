...
public class DeltaJConstraintsGenerator implements IGenerator {
  ...
  public void doGenerate(final Resource resource, final IFileSystemAccess fsa) {
    Iterable<DeltaModule> _filter = Iterables.<DeltaModule>filter(IteratorExtensions.<EObject>toIterable(resource.getAllContents()), DeltaModule.class);
  ...
  public void compile(final DeltaModule delta, final IFileSystemAccess fsa) {
    ...
    fsa.generateFile(_plus_1, this.compile(delta));
  }
  ...
  protected CharSequence _compile(final ClassAddition action) {
    ...
    String _name = action.getClass_().getName();
    _builder.append(_name);
    _builder.append(" with {");
    _builder.newLineIfNotEmpty();
    {
      EList<Method> _methods = action.getClass_().getMethods();
  ...
  public CharSequence compile(final Method method) {
    ...
    _builder.newLineIfNotEmpty();
    _builder.append("\t");
    CharSequence _genConstraints = this.statementConstraintGenerator.genConstraints(this.typeSystem, method.getBody().getStatements());
