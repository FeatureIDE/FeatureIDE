...
public class DeltaJConstraintGeneratorHelper {
  ...
  protected CharSequence _genComment(final JavaVerbatim javaVerbatim) {
    ...
    return IterableExtensions.join(ListExtensions.<String, String>map(this._deltaJGeneratorExtensions.javaVerbatimLines(javaVerbatim), _function), "\n");
  }
  ...
  public CharSequence genMethodConstraint(final Type receiverType, final String methodName, final Type returnType, final List<Type> typesForParams, final Expression exp) {
    ...
    final Function1<Type, CharSequence> _function = (Type t) -> {
      return this.typeGenerator.compileType(t);
    };
    String _join = IterableExtensions.join(ListExtensions.<Type, CharSequence>map(typesForParams, _function), ", ");
