...
constraint returns [Equation value]
@init {
List<WeightedTerm> wTerms = new ArrayList<WeightedTerm>();
}
	: sum[wTerms] RELATION degree SEMICOLON EOF {$value = new Equation(wTerms, $RELATION.text, $degree.value);};
...
degree returns [int value]
	: sign unsigned {$value = Integer.parseInt($unsigned.text)*$sign.value;}
	| unsigned {$value = Integer.parseInt($unsigned.text);};
