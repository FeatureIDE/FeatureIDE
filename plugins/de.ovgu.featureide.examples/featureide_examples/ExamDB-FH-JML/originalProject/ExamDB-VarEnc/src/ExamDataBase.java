...
// code from feature Statistics
     ...
     */    
    /*@ public normal_behavior
      @  requires FeatureModel.f(FeatureModel.Statistics);
      @  ensures \result==(\num_of int i; 
      @                        0<=i && i<students.length; validStudent(students[i]));
      @*/
     ...
     */ 
    /*@ public normal_behavior
      @  requires FeatureModel.f(FeatureModel.Statistics) && consistent();
      @  assignable \nothing;
      @  ensures \result==(\num_of int i; 
      @                       0<=i && i<students.length; validStudent(students[i])
...
// code from feature BackOut
     ...
     */
    /*@ public normal_behavior
      @  requires FeatureModel.f(FeatureModel.BackOut) && (\exists int i; 
      @                0<=i && i<students.length && students[i]!=null
      @                && students[i].matrNr==matrNr);
...
// code from feature BonusPoints
    
    /*@ public invariant
    @ !FeatureModel.f(FeatureModel.BackOut) || (\forall int i;  
    @          0<=i && i<students.length && students[i]!=null;
    @              (\forall ExamDataBase ex; ex!=null && ex!=this; (\forall int k;
    @                   0<=k && k<ex.students.length; ex.students[k]!=students[i]))
...
//code merged from features ExamDB and BackOut
  
  private /*@pure helper@*/ boolean validStudent__ExamDB(Student student) {
  	return student != null;
  }
