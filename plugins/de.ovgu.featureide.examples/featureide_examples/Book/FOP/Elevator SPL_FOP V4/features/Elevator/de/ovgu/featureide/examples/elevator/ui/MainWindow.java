...
public class MainWindow implements ITickListener {
	...
	private List<FloorComposite> listFloorComposites =
		new ArrayList<FloorComposite>();
	...
	public void initialize(int maxFloors) {
		...
		frmElevatorSample.setBounds(100, 50, 900, 650); // window position and
														 // size
	...
	private void createPanelControlsContent(int maxFloors) {
		...
		try {
			panel_control =
				new JBackgroundPanel(MainWindow.class.getResourceAsStream("/elevator_inside2.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		splitPane.setRightComponent(panel_control);

		GridBagLayout gbl_panel_control =
			new GridBagLayout();
		...
		GridBagConstraints gbc_lbl =
			new GridBagConstraints();
		gbc_lbl.gridwidth =
			4;
		gbc_lbl.insets =
			new Insets(0, 0, 185, 0);
		gbc_lbl.fill =
		...
		addElevatorControlButtons(maxFloors);
	}

	private void addElevatorControlButtons(int maxFloors) {

	}

	private void addBuilding(int maxFloors) {
		JPanel panel_building =
			new JPanel();
		...
		JScrollPane scrollPane =
			new JScrollPane(panel_building);
	...
	public void onTick(Elevator elevator) {
		...
		switch (state) {
		case MOVING_UP:
			this.listFloorComposites.get(currentFloor
				- 1).showImageClose();
			break;
		case MOVING_DOWN:
			this.listFloorComposites.get(currentFloor
