	...
	private static void convertConfigFiles(ProjectRecord projectRecord) {
		if (!projectRecord.hasErrors() && !projectRecord.hasWarnings()) {
			...
			if (!configurationFiles.isEmpty()) {
				...
				final IFeatureModel featureModel = FeatureModelManager.load(relativePath.resolve("model.xml"), problems);
				if (!problems.containsError()) {
					final Configuration object = new Configuration(featureModel);
