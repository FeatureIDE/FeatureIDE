...
public abstract class ADocumentationCommentParser {
	...
	protected int curFeatureID, curInfoType, curPriority;
	
	protected final List<BlockTag>
		generalTags = new ArrayList<BlockTag>(),
		featureTags = new ArrayList<BlockTag>();
	...
	private void addBlockTagToList(String comment) {
		final List<BlockTag> tagList = (curInfoType == 0) ? generalTags : featureTags;
	...
	private BlockTag categorize(String head, String body) {
		...
		tag.setInformationType(curInfoType);
		tag.setFeatureID(curFeatureID);
		return tag;
	}
