...
public class VerticalLayout extends FeatureDiagramLayoutManager {
	...
	private void centerChildren(LayoutableFeature feature, int level) {
		...
		if (feature.hasChildren()) {
			...
			Feature f = feature.getFeature();
			FeatureUIHelper.setLocation(f, new Point(positionsX.get(level), positionsY.get(childlessNum)));
			childlessNum++;
			yOffset = FeatureUIHelper.getLocation(f).y + FeatureUIHelper.getSize(f).height + FMPropertyManager.getFeatureSpaceX();
		}
	...
	private void centerOther(int level) {
		for (int i = 0; i < levelList.get(level).size(); i++) {
			...
			if (feature.hasChildren()) {
				int yPos = (FeatureUIHelper.getLocation(feature.getFirstChild().getFeature()).y + FeatureUIHelper.getLocation(feature.getLastChild()
						.getFeature()).y) / 2;
