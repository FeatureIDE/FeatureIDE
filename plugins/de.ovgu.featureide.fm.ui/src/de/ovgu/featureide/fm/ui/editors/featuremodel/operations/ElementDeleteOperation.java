...
public class ElementDeleteOperation extends MultiFeatureModelOperation implements GUIDefaults {
	...
	public void createSingleOperations(IFeatureModel featureModel) {
		...
		for (final Object element : getSelection().toArray()) {
			...
			final IFeature parent = createFeatureDeleteOperation(element, removalMap, alreadyDeleted);
	...
	private IFeature createFeatureDeleteOperation(Object element, Map<IFeature, List<IFeature>> removalMap, List<IFeature> alreadyDeleted) {
		...
		if (feature != null) {
			...
			if (FeatureUtils.getRelevantConstraints(feature).isEmpty()) {
				...
				final FeatureDependencies featureDependencies = new FeatureDependencies(featureModelManager.editObject(), false);
				final List<IFeature> equivalent = new LinkedList<>();
