...
public class FullViewportAutoexposeHelper extends ViewportAutoexposeHelper {
	...
	public boolean step(Point point) {
		...
		final Point oldLoc = port.getViewLocation();
		final Point newLoc = getNewLocation(oldLoc, getInnerBounds().getPosition(point), getOffset());
	...
	private Rectangle getInnerBounds() {
		...
		final Rectangle rect = Rectangle.SINGLETON.getCopy();
	...
	private Point getNewLocation(Point old, int direction, int scrollOffset) {
		...
		if ((direction & PositionConstants.EAST) != 0) {
			loc.x += scrollOffset;
		} else if ((direction & PositionConstants.WEST) != 0) {
			loc.x -= scrollOffset;
		}
