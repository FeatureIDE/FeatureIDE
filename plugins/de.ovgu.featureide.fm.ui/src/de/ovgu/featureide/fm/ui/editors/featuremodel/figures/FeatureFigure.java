...
public class FeatureFigure extends ModelElementFigure implements GUIDefaults {
	...
	public FeatureFigure(IGraphicalFeature feature, IGraphicalFeatureModel featureModel) {
		...
		// Checks if manual Layout is chosen and a feature is not positioned yet
		if ((featureModel.getLayout().getLayoutAlgorithm() == 0) && feature.getLocation().equals(new Point(-1, -1))) {
			adjustManualLayoutLocation(feature);
		}
	}

	// adjust the position of a new feature when manual layout is chosen
	private void adjustManualLayoutLocation(IGraphicalFeature graphicalFeature) {
		final IGraphicalFeature parent = graphicalFeature.getSourceConnection().getTarget();
		// decides if the feature is created below or above
		if (graphicalFeature.getGraphicalChildren(true).size() == 0) {
			if (parent != null) {
				featureBelow(parent, graphicalFeature);
			}
		} else {
			featureAbove(graphicalFeature);
		}
	}

	private void featureAbove(IGraphicalFeature graphicalFeature) {
		...
		int yLocation = children.get(0).getLocation().y - graphicalFeature.getSize().height - 18;
		int xLocation = children.get(0).getLocation().x - graphicalFeature.getSize().width;
		...
		for (final IGraphicalFeature child : children) {
			if (child.getLocation().x < minX) {
				...
				yLocation = child.getLocation().y - child.getSize().height - 18;
			}
			if (child.getLocation().y < minY) {
				minY = child.getLocation().y;
				xLocation = child.getLocation().x - child.getSize().width;
			}
		}
		// decides if the anchor points are at the side or on the top of the rectangle
		if (sourceAnchor.getLocation(graphicalFeature.getLocation()).y == graphicalFeature.getLocation().y) {
	...
	private void featureBelow(IGraphicalFeature localParent, IGraphicalFeature graphicalFeature) {
		int maxX = localParent.getLocation().x;
		int maxY = localParent.getLocation().y;
		...
		// decides if the anchor points are at the side or on the top of the rectangle
		if (sourceAnchor.getLocation(graphicalFeature.getLocation()).y == graphicalFeature.getLocation().y) {
			graphicalFeature.setLocation(new Point(maxX + 8, yLocation));
		} else {
			graphicalFeature.setLocation(new Point(xLocation, maxY));
		}
	}
