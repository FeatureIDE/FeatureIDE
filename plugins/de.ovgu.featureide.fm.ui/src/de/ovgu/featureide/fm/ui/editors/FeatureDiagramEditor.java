...
public class FeatureDiagramEditor extends ScrollingGraphicalViewer implements GUIDefaults, IEventListener, IFeatureModelEditorPage {
	...
	public void initializeGraphicalViewer() {
		getControl().addControlListener(new ControlListener() {
			...
			public void controlResized(ControlEvent e) {
				...
				org.eclipse.draw2d.geometry.Point oldLoc =
					object.getLocation();
				if (oldLoc == null)
					return;
				internRefresh(true);
	...
	private void fillContextMenu(IMenuManager menu) {
		IMenuManager subMenuCalculations =
	...
	public void propertyChange(FeatureIDEEvent event) {
		...
		case FEATURE_ADD_ABOVE:
			...
			if (event.getNewValue() != null
				...
				for (IGraphicalFeature child : graphicalFeatureModel.getGraphicalFeature(newCompound)
		...
		case FEATURE_ADD:
			...
			if (parent != null
				...
				IGraphicalFeature graphicalParent =
				...
				if (graphicalParent.isCollapsed()) {
					...
					for (IFeatureStructure featureStructure : parent.getStructure().getChildren()) {
						if (featureStructure != newFeature.getStructure()) {
							IGraphicalFeature graphicalFeatureStructure =
		...
		case MODEL_DATA_CHANGED:
			...
			break;
		case FEATURE_DELETE:
			IGraphicalFeature deletedFeature =
			...
			if (oldParent != null) {
				...
				for (IGraphicalFeature child : graphicalFeatureModel.getGraphicalFeature(oldParent)
						.getGraphicalChildren(graphicalFeatureModel.getLayout().showHiddenFeatures())) {
					child.update(FeatureIDEEvent.getDefault(EventType.PARENT_CHANGED));
				}
				refreshChildAll(oldParent);
				...
				for (Object obj : getEditPartRegistry().values()) {
					if (obj instanceof LegendEditPart) {
						LegendFigure fig =
							((LegendEditPart) obj).getFigure();
						fig.recreateLegend();
						org.eclipse.draw2d.geometry.Point newLegendPosition =
	...
	public void centerPointOnScreen(IFeature feature) {
		...
		IGraphicalFeature graphFeature =
			graphicalFeatureModel.getGraphicalFeature(feature);
