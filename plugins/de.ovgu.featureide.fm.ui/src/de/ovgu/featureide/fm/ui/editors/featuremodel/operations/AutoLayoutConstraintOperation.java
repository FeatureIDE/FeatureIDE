...
public class AutoLayoutConstraintOperation extends AbstractGraphicalFeatureModelOperation {
	...
	private int counter;
	private LinkedList<LinkedList<Point>> oldPos =
	...
	protected FeatureIDEEvent operation() {
		List<IGraphicalConstraint> constraintList =
			graphicalFeatureModel.getConstraints();
		int minX =
			Integer.MAX_VALUE;
		int maxX =
			0;
		if (!constraintList.isEmpty()) {
			...
			LinkedList<IGraphicalFeature> featureList =
				new LinkedList<>();
		...
		for (int i =
			...
			newPos.x =
				(minX
					+ maxX)
					/ 2
					- constraintList.get(i).getSize().width
						/ 2;
			newPos.y =
