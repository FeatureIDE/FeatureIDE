...
public class CollaborationView extends ViewPart implements GUIDefaults, ICurrentBuildListener, ISaveablePart {
	...
	public static final String ID =
		UIPlugin.PLUGIN_ID
			+ ".views.collaboration.Collaboration";

	private static final String OPEN_MESSAGE =
		OPEN_A_FILE_FROM_A_FEATUREIDE_PROJECT;
	...
	private static final String UNSELECTED_LABEL =
		SHOW_UNSELECTED_FEATURES;
	private static final String EXPORT_AS_LABEL =
		EXPORT_AS;

	private static final String REFRESH_TOOL_TIP_LABEL =
		BUILD_COLLABORATIONMODEL;

	private static final String[] FIELD_METHOD_LABEL_NAMES =
		{
			FIELDS_WITH_REFINEMENTS,
			FIELDS_WITHOUT_REFINEMENTS,
			METHODS_WITH_REFINEMENTS,
			METHODS_WITHOUT_REFINEMENTS,
			SHOW_METHOD_CONTRACTS,
			SHOW_CLASS_INVARIANTS,
			SHOW_NESTED_CLASSES,
			HIDE_PARAMETER_TYPES,
			PUBLIC,
			PROTECTED,
			DEFAULT,
			PRIVATE,
	...
	private static final Image[] FIELD_METHOD_IMAGES =
		{
			IMAGE_FIELDS_REFINEMENTS,
			IMAGE_FIELDS_WITHOUT_REFINEMENTS,
	...
	private final CollaborationModelBuilder builder =
		new CollaborationModelBuilder();
	...
	private final SetColorAction[] setColorActions =
		new SetColorAction[ColorPalette.COLOR_COUNT];
	private final ShowFieldsMethodsAction[] setFieldsMethodsActions =
		new ShowFieldsMethodsAction[FIELD_METHOD_LABEL_NAMES.length];

	private ShowFieldsMethodsAction showAccessModifiers;

	private IToolBarManager toolbarManager;

	private final Vector<IFile> configurations =
		new Vector<IFile>();
	private final Job updateGUIJob =
		new AWaitingJob(UPDATE_COLLABORATION_VIEW) {
			...
			public IStatus execute(IProgressMonitor monitor) {
				...
				final FSTModel model =
					builder.buildCollaborationModel(CorePlugin.getFeatureProject(configurationFile));
				if (model == null) {
					refreshButton.setEnabled(true);
					return Status.OK_STATUS;
				}
				...
				final UIJob uiJob =
					new UIJob(UPDATE_COLLABORATION_VIEW) {
	...
	private final IEventListener colorChangeListener =
		new IEventListener() {
	...
	private void setFeatureProject(IFeatureProject featureProject) {
		if (this.featureProject != featureProject) {
			...
			if (this.featureProject != null) {
				featureModel =
					this.featureProject.getFeatureModel();
	...
	public void createPartControl(Composite parent) {
		...
		showAccessModifiers =
			new ShowFieldsMethodsAction("", null, this, select, IAction.AS_DROP_DOWN_MENU);
		...
		showAccessModifiers.setMenuCreator(new IMenuCreator() {
			...
			public Menu getMenu(Control parent) {
				fMenu =
					new Menu(parent);
				for (int i =
					ShowFieldsMethodsAction.PUBLIC_FIELDSMETHODS; i <= ShowFieldsMethodsAction.PRIVATE_FIELDSMETHODS; i++) {
					final ShowFieldsMethodsAction action =
						setFieldsMethodsActions[i];
				...
				final ShowFieldsMethodsAction selectAll =
					new ShowFieldsMethodsAction(SELECT_ALL, IMAGE_SELECT_ALL_MODIFIERS, CollaborationView.this,
							ShowFieldsMethodsAction.SELECT_ALL_METHOD_ACCESS);
				final ActionContributionItem contributionItem =
					new ActionContributionItem(selectAll);
				contributionItem.fill(fMenu, -1);

				final ShowFieldsMethodsAction deselectAll =
					new ShowFieldsMethodsAction(DESELECT_ALL, IMAGE_MODIFIERS_NONE, CollaborationView.this,
							ShowFieldsMethodsAction.DESELECT_ALL_METHOD_ACCESS);
				final ActionContributionItem deselectAllConteribution =
					new ActionContributionItem(deselectAll);
		...
		final CollaborationViewSearch.Builder builder =
			new CollaborationViewSearch.Builder();
		search =
			builder.setAttachedViewerParent(viewer).setSearchBoxText(SEARCH_IN_COLLABORATION_DIAGRAM).setFindResultsColor(ROLE_BACKGROUND_SELECTED)
	...
	private void setEditorActions(IWorkbenchPart activeWorkbenchPart) {
		...
		if (activeWorkbenchPart == null) {
			...
			activeEditor =
				(IEditorPart) activeWorkbenchPart;
			currentEditor =
				activeWorkbenchPart;
		} else {
			final IWorkbenchPage page =
				activeWorkbenchPart.getSite().getPage();
			if (page != null) {
				activeEditor =
					page.getActiveEditor();
			}
		...
		if ((activeEditor != null)
			&& (activeEditor.getEditorInput() instanceof FileEditorInput)) {
			// case: open editor
			final IFile inputFile =
				((FileEditorInput) activeEditor.getEditorInput()).getFile();
			...
			if (featureProject != null) {
				...
				if (ConfigFormatManager.getInstance().hasFormat(inputFile.getName())) {
					...
					if ((CollaborationModelBuilder.editorFile != null)
						&& CollaborationModelBuilder.editorFile.getName().equals(inputFile.getName())
	...
	public void createContextMenu() {
		...
		final Control control =
			viewer.getControl();
		final Menu menu =
			menuMgr.createContextMenu(control);
	...
	private void fillContextMenu(IMenuManager menuMgr) {
		...
		if (isNotEmpty
			...
			final StructuredSelection selectionAsStructuredSelection =
				(StructuredSelection) selection;

			final Iterator<?> iterator =
				selectionAsStructuredSelection.iterator();
			Object selectedItem =
				null;
			boolean onlyCollaborations =
				true;
		...
		if (featureProject.getComposer().showContextFieldsAndMethods()) {
			final MenuManager methodsFieldsSubMenu =
				new MenuManager(SHOW_FIELDS_AND_METHODS);

			for (int i =
				0; i < setFieldsMethodsActions.length; i++) {
		...
		final MenuManager exportMenu =
			new MenuManager(EXPORT_AS_LABEL);
	...
	public void disableToolbarFilterItems() {
		...
		if (featureProject != null) {
			...
			if (composer != null) {
				value =
					composer.showContextFieldsAndMethods();
			}
		}
	...
	private void createActions(IEditorPart part) {
		addRoleAction =
			new AddRoleAction(ADD_LABEL, viewer, this);
		delAction =
			new DeleteAction(DELETE_LABEL, viewer);
		filterAction =
			new FilterAction(FILTER_LABEL, viewer, this);
		showUnselectedAction =
			new ShowUnselectedAction(UNSELECTED_LABEL, this);
		setFeatureColourAction =
			new SetFeatureColorAction(viewer);
		for (int i =
			0; i < FIELD_METHOD_LABEL_NAMES.length; i++) {
			setFieldsMethodsActions[i] =
				new ShowFieldsMethodsAction(FIELD_METHOD_LABEL_NAMES[i], FIELD_METHOD_IMAGES[i], this, i);
		}
		...
		for (int i =
			0; i < setColorActions.length; i++) {
			setColorActions[i] =
				new SetColorAction(viewer, this, i);
		}
	}

	private boolean deselectAll() {
		final boolean[] selectedModifiers =
			RoleFigure.getRoleSelections();
		if ((selectedModifiers[ShowFieldsMethodsAction.PUBLIC_FIELDSMETHODS])
			&& (selectedModifiers[ShowFieldsMethodsAction.PRIVATE_FIELDSMETHODS])
			&& (selectedModifiers[ShowFieldsMethodsAction.PROTECTED_FIELDSMETHODS])
			&& (selectedModifiers[ShowFieldsMethodsAction.DEFAULT_FIELDSMETHODS])) {
			return true;
		}
		return false;
	}
	...
	private ImageDescriptor getAccessModifiersImage(boolean[] selectedModifiers) {
		final ArrayList<Image> arrayList =
			new ArrayList<Image>();
		if (selectedModifiers[ShowFieldsMethodsAction.PUBLIC_FIELDSMETHODS]) {
		...
		final Image combinedImages =
			combineImages(arrayList);
	...
	public class ImageComarator implements Comparator<Image> {
		...
		public int compare(Image image1, Image image2) {
			return image1.getImageData().bytesPerLine > image2.getImageData().bytesPerLine
				? 1
				: 0;
		}
	}
	...
	private Image combineImages(ArrayList<Image> images) {
		...
		final Image finalImage =
			new Image(images.get(0).getDevice(), images.get(0).getBounds());
		final GC gc =
			new GC(finalImage);
		int x =
			0;
		...
		for (int j =
			...
			gc.drawImage(images.get(j), 4, 4, images.get(j).getBounds().width
				- (2
					* 4), images.get(j).getBounds().height
						- (2
							* 4),
					x, y,
					finalImage.getBounds().width
						/ 2,
					finalImage.getBounds().height
						/ 2);
	...
	public void reloadImage() {
		...
		final boolean[] selectedAccessModifiers =
			RoleFigure.getRoleSelections();
	...
	private void fillLocalToolBar() {
		...
		fieldsWithRefinementsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.FIELDS_WITH_REFINEMENTS];
		fieldsWithoutRefinementsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.FIELDS_WITHOUT_REFINEMENTS];
		methodsWithRefinementsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.METHODS_WITH_REFINEMENTS];
		methodsWithoutRefinementsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.METHODS_WITHOUT_REFINEMENTS];
		showContracsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.ONLY_CONTRACTS];
		showInvariantsButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.ONLY_INVARIANTS];
		showNestedClassesButton =
			setFieldsMethodsActions[ShowFieldsMethodsAction.SHOW_NESTED_CLASSES];

		final boolean[] isChecked =
			RoleFigure.getRoleSelections();
		for (int i =
			ShowFieldsMethodsAction.FIELDS_WITH_REFINEMENTS; i <= ShowFieldsMethodsAction.SHOW_NESTED_CLASSES; i++) {
		...
		exportAsToolbarIcon.setMenuCreator(new IMenuCreator() {
			...
			public Menu getMenu(Control parent) {
				...
				final ActionContributionItem exportImageContributionItem =
					new ActionContributionItem(exportAsImage);
				exportImageContributionItem.fill(fMenu, -1);
				final ActionContributionItem exportXMLContributionItem =
					new ActionContributionItem(exportAsXML);
			...
			new Action() {
				...
				public void run() {
					...
					final LongRunningJob<Boolean> runner =
						new LongRunningJob<>(REFRESH_COLLABORATION_VIEW, job);
	...
	public void refresh() {
		...
		final FSTModel model =
			builder.buildCollaborationModel(featureProject);
