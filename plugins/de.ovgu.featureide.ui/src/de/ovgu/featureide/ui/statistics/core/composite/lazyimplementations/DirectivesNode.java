...
public class DirectivesNode extends LazyParent {
	...
	protected void initChildren() {
		...
		directivesPerClass.addChild(new JumpNode(MAXIMUM_NUMBER_OF_DIRECTIVES + maximumSum.getValue() + IN_CLASS + maximumSum.getKey(), fstModel.getFeatureProject(), maximumSum.getKey()));
		// 1.1.2.2 Minimum number of directives:
		directivesPerClass.addChild(new JumpNode(MINIMUM_NUMBER_OF_DIRECTIVES + minimumSum.getValue() + IN_CLASS + minimumSum.getKey(), fstModel.getFeatureProject(), minimumSum.getKey()));
		...
		featuresPerDirectives.addChild(new JumpNode(MAXIMUM_FEATURES_PER_DIRECTIVE, maximumNumberOfFeatures.getValue() + IN_CLASS + maximumNumberOfFeatures.getKey(), fstModel.getFeatureProject(), maximumNumberOfFeatures.getKey()));
		// 1.1.3.2 Maximum number of features per directive:
		featuresPerDirectives.addChild(new JumpNode(MINIMUM_FEATURES_PER_DIRECTIVE, minimumNumberOfFeatures.getValue() + IN_CLASS + minimumNumberOfFeatures.getKey(), fstModel.getFeatureProject(), minimumNumberOfFeatures.getKey()));
		...
		project.addChild(new JumpNode(MAXIMUM_NESTING_OF_DIRECTIVES, maxNesting.getValue() + IN_CLASS + maxNesting.getKey(), fstModel.getFeatureProject(), maxNesting.getKey(), line));
		...
		Parent classes = new AbstractSortModeNode(CLASS_STATISTICS) {
			...
			protected void initChildren() {
				for(FSTClass c : fstModel.getClasses()){
					...
					p.addChild(new JumpNode(MAXIMUM_NESTING_OF_DIRECTIVES, aggProject.getNestingCountForClass(c.getName()), fstModel.getFeatureProject(), c.getName(), line));
					addChild(p);
				}
