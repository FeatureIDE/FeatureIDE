...
public class ConfigDialog extends TitleAreaDialog {
	...
	private void createConverter() {
	    ...
	    GridData gd_text_1 = new GridData(SWT.RIGHT, SWT.TOP, false, false, 1, 1);
	...
	private void createTimeOutComboBox() {
	    ...
	    GridData gd_timeOutComboBox = new GridData(SWT.RIGHT, SWT.TOP, false, false, 1, 1);
	...
	private void convertToReadable() {
		try {
			long time = Long.parseLong(timeOutComboBox.getText());
			converterToMinutes.setText(String.format("%02d h %02d min %02d sec", TimeUnit.SECONDS.toHours(time), TimeUnit.SECONDS.toMinutes(time)
					- TimeUnit.HOURS.toMinutes(TimeUnit.SECONDS.toHours(time)), time - TimeUnit.MINUTES.toSeconds(TimeUnit.SECONDS.toMinutes(time))));
	...
	protected void okPressed() {
		String p = priorityComboBox.getText();
		if (p.equals(LOW)) {
