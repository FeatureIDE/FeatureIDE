...
/**
 * The PBSolver interface encapsulates the functionality of a pseudo boolean satisfiability solver. Each class that implements this interface can be plugged in
 * to reason on extended feature models.
 *
...
public interface PBSolver {
	 ...
	 *
	 * @return True if there is at least one satisfying assignments, false otherwise.
	 */
	public boolean isSatisfiable();

	/**
	 * Is the currently submitted set of restrictions satisfiable with respect of the passed assumptions?
	 *
	...
	/**
	 * Verifies if the passed variables are either true or false for all satisfying assignments. Returns +id for each true assignments and -id for each false
	 * assignment in the resulting set. In general, the resulting set contains fewer elements than the input set because some variables are not yet statically
	 * assigned.
	 *
