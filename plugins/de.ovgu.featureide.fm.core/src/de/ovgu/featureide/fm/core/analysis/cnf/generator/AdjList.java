 ...
 */
public class AdjList implements IModalImplicationGraph {
	...
	public static class Traverser implements ITraverser {
		...
		public VecInt traverse2(int curVar, int[] model) {
		...
		@Override
		public void traverseDefined(LiteralSet newVars) {
			Arrays.fill(dfsMark, (byte) 0);

			for (int var : newVars.getLiterals()) {
				changed.push(var);
			}
			for (int var : newVars.getLiterals()) {
				traverseStrong(var);
		...
		private void traverseWeak(int curVar) {
			...
			outerLoop: for (int i = 0; i < complexClauses.length; i++) {
				final LiteralSet clause = adjList.complexClauses.get(complexClauses[i]);
