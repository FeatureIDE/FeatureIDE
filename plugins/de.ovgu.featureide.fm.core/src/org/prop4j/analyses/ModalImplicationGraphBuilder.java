...
import org.prop4j.analyses.AdjList.Vertex;
...
public class ModalImplicationGraphBuilder implements LongRunningMethod<AdjList> {
	...
	private ModalImplicationGraphBuilder(SatInstance satInstance, boolean detectStrong) {
	...
	private boolean addStrongRelation(final int signedVarX, final int signedVarY) {
		...
		if (signedVarX > 0) {
			if (signedVarY > 0) {
				adjMatrix.edges[combinationIndexXY] = (byte) ((oldXY & (~IFeatureGraph2.EDGE_NEGATIVE)) | IFeatureGraph2.EDGE_01);
				adjMatrix.edges[combinationIndexYX] = (byte) ((oldYX & (~IFeatureGraph2.EDGE_NEGATIVE)) | IFeatureGraph2.EDGE_01);
			} else {
				adjMatrix.edges[combinationIndexXY] = (byte) ((oldXY & (~IFeatureGraph2.EDGE_NEGATIVE)) | IFeatureGraph2.EDGE_00);
				adjMatrix.edges[combinationIndexYX] = (byte) ((oldYX & (~IFeatureGraph2.EDGE_POSITIVE)) | IFeatureGraph2.EDGE_11);
			}
		} else {
			if (signedVarY > 0) {
				adjMatrix.edges[combinationIndexXY] = (byte) ((oldXY & (~IFeatureGraph2.EDGE_POSITIVE)) | IFeatureGraph2.EDGE_11);
				adjMatrix.edges[combinationIndexYX] = (byte) ((oldYX & (~IFeatureGraph2.EDGE_NEGATIVE)) | IFeatureGraph2.EDGE_00);
			} else {
				adjMatrix.edges[combinationIndexXY] = (byte) ((oldXY & (~IFeatureGraph2.EDGE_POSITIVE)) | IFeatureGraph2.EDGE_10);
				adjMatrix.edges[combinationIndexYX] = (byte) ((oldYX & (~IFeatureGraph2.EDGE_POSITIVE)) | IFeatureGraph2.EDGE_10);
			}
		}
	...
	private void addWeakRelation(final int signedVarX, final int signedVarY) {
		...
		if (signedVarX > 0) {
			if (signedVarY > 0) {
				if ((oldXY & IFeatureGraph2.EDGE_STRONG_NEGATIVE) == 0) {
					adjMatrix.edges[combinationIndexXY] |= IFeatureGraph2.EDGE_01Q;
				}
				if ((oldYX & IFeatureGraph2.EDGE_STRONG_NEGATIVE) == 0) {
					adjMatrix.edges[combinationIndexYX] |= IFeatureGraph2.EDGE_01Q;
				}
			} else {
				if ((oldXY & IFeatureGraph2.EDGE_STRONG_NEGATIVE) == 0) {
					adjMatrix.edges[combinationIndexXY] |= IFeatureGraph2.EDGE_00Q;
				}
				if ((oldYX & IFeatureGraph2.EDGE_STRONG_POSITIVE) == 0) {
					adjMatrix.edges[combinationIndexYX] |= IFeatureGraph2.EDGE_11Q;
				}
			}
		} else {
			if (signedVarY > 0) {
				if ((oldXY & IFeatureGraph2.EDGE_STRONG_POSITIVE) == 0) {
					adjMatrix.edges[combinationIndexXY] |= IFeatureGraph2.EDGE_11Q;
				}
				if ((oldYX & IFeatureGraph2.EDGE_STRONG_NEGATIVE) == 0) {
					adjMatrix.edges[combinationIndexYX] |= IFeatureGraph2.EDGE_00Q;
				}
			} else {
				if ((oldXY & IFeatureGraph2.EDGE_STRONG_POSITIVE) == 0) {
					adjMatrix.edges[combinationIndexXY] |= IFeatureGraph2.EDGE_10Q;
				}
				if ((oldYX & IFeatureGraph2.EDGE_STRONG_POSITIVE) == 0) {
					adjMatrix.edges[combinationIndexYX] |= IFeatureGraph2.EDGE_10Q;
				}
			}
		}
	}
	...
	private ArrayList<TempVertex> createTempVertices(final List<Clause> clauseList) {
		...
		for (int i = 0; i < numberOfVariables; i++) {
			...
			for (int j = 0; j < numberOfVariables; j++) {
				...
				if ((relation & IFeatureGraph2.EDGE_00) != 0) {
					vertex.negStrongEdges.add(-(j + 1));
				} else if ((relation & IFeatureGraph2.EDGE_01) != 0) {
					vertex.negStrongEdges.add((j + 1));
				}
				if ((relation & IFeatureGraph2.EDGE_10) != 0) {
					vertex.posStrongEdges.add(-(j + 1));
				} else if ((relation & IFeatureGraph2.EDGE_11) != 0) {
					vertex.posStrongEdges.add((j + 1));
				}
			}
		}
	...
	private void dfsStrong(int curVar) {
		...
		for (int nextIndex = 0; nextIndex < adjMatrix.getNumVariables(); nextIndex++) {
			...
			if ((bitMask & IFeatureGraph2.EDGE_00) != 0) {
				dfsStrong(-(nextIndex + 1));
			} else if ((bitMask & IFeatureGraph2.EDGE_01) != 0) {
				dfsStrong((nextIndex + 1));
			}
		}
		dfsStack.removeLast();
	...
	private void dfsWeak(int curVar) {
		...
		for (int nextIndex = 0; nextIndex < adjMatrix.getNumVariables(); nextIndex++) {
			...
			if ((bitMask & IFeatureGraph2.EDGE_00) != 0) {
				...
				if ((bitMask & IFeatureGraph2.EDGE_00Q) != 0) {
					dfsWeak(-(nextIndex + 1));
				}
				if ((bitMask & IFeatureGraph2.EDGE_01Q) != 0) {
					dfsWeak((nextIndex + 1));
				}
			}
		}
		...
		if ((adjMatrix.core[i] == 0) && ((dfsMark[i] & compareB) == 0)) {
			...
			inner1: for (int j = i + 1; j < xModel1.length; j++) {
				...
				if ((adjMatrix.core[j] == 0)
						&& ((positive && ((b & IFeatureGraph2.EDGE_WEAK_POSITIVE) != 0)) || (!positive && ((b & IFeatureGraph2.EDGE_WEAK_NEGATIVE) != 0)))) {
