...
public class FeatureModelAnalysis implements LongRunningMethod<HashMap<Object, Object>> {
	...
	private final HashMap<Object, Object> changedAttributes =
		new HashMap<>();
	...
	public void setCalculateRedundantConstraints(boolean calculateRedundantConstraints) {
		this.calculateRedundantConstraints =
			calculateRedundantConstraints;
	}

	public void setCalculateTautologyConstraints(boolean calculateTautologyConstraints) {
		this.calculateTautologyConstraints =
			calculateTautologyConstraints;
	}

	public void setCalculateFOConstraints(boolean calculateFOConstraints) {
		this.calculateFOConstraints =
			calculateFOConstraints;
	}

	public void setCalculateDeadConstraints(boolean calculateDeadConstraints) {
		this.calculateDeadConstraints =
			calculateDeadConstraints;
	}
	...
	public void updateFeatures() {
		...
		final SatInstance si =
			new SatInstance(nodeCreator.createNodes(), FeatureUtils.getFeatureNamesPreorder(fm));
	...
	private void checkConstraintDeadAndFalseOptional(final List<IConstraint> constraints) throws ContradictionException {
		...
		final SatInstance si =
			new SatInstance(nodeCreator.createNodes(), FeatureUtils.getFeatureNamesPreorder(fm));
		final BasicSolver modSat =
			new BasicSolver(si);

		final List<IFeature> deadList =
			new LinkedList<>(deadFeatures);
		final List<IFeature> foList =
			new LinkedList<>(falseOptionalFeatures);
		monitor.checkCancel();
		...
		for (final IConstraint constraint : constraints) {
			...
			if (constraint.getConstraintAttribute() == ConstraintAttribute.NORMAL) {
				if (calculateDeadConstraints) {
					final List<IFeature> newDeadFeature =
						checkFeatureDead2(modSat, deadList);
				...
				if (calculateFOConstraints) {
					final List<IFeature> newFOFeature =
						checkFeatureFalseOptional2(modSat, foList);
	...
	private void checkConstraintRedundant(final List<IConstraint> constraints) throws ContradictionException {
		if (calculateRedundantConstraints) {
			...
			final SatInstance si =
				new SatInstance(nodeCreator.createNodes(), FeatureUtils.getFeatureNamesPreorder(fm));
			final ModifiableSolver redundantSat =
				new ModifiableSolver(si);

			final List<List<IConstr>> constraintMarkers =
				new ArrayList<>();
			final List<Node> cnfNodes =
				new ArrayList<>();
			for (final IConstraint constraint : constraints) {
				final Node cnf =
					constraint.getNode().toRegularCNF();
				cnfNodes.add(cnf);
	...
	private void checkConstraintUnsatisfiable(final List<IConstraint> constraints) throws ContradictionException {
		...
		for (final IConstraint constraint : constraints) {
			...
			List<IConstr> constraintMarkers =
				null;
			boolean satisfiable;
			try {
				constraintMarkers =
					unsat.addClauses(cnf);
				satisfiable =
					unsat.isSatisfiable() == SatResult.TRUE;
			} catch (final ContradictionException e) {
				satisfiable =
					false;
			}
	...
	private void checkFeatureDead(final SatInstance si) {
		...
		coreFeatures.clear();
		final int[] solution2 =
			LongRunningWrapper.runMethod(new CoreDeadAnalysis(si), monitor.subTask(0));
		monitor.checkCancel();
		for (int i =
			0; i < solution2.length; i++) {
			monitor.checkCancel();
			final int var =
				solution2[i];
			final IFeature feature =
				fm.getFeature((String) si.getVariableObject(var));
			if (var < 0) {
	...
	private List<IFeature> checkFeatureDead2(final BasicSolver solver, List<IFeature> deadList) {
		...
		final List<IFeature> result =
			new ArrayList<>();
		final int[] deadVar =
			new int[deadList.size()];
		int j =
			0;
		for (final IFeature deadFeature : deadList) {
			deadVar[j++] =
				solver.getSatInstance().getVariable(deadFeature.getName());
		}
		final int[] solution2 =
			LongRunningWrapper.runMethod(new CoreDeadAnalysis(solver, deadVar));
		for (int i =
			0; i < solution2.length; i++) {
			final int var =
				solution2[i];
	...
	private void checkFeatureFalseOptional(final Iterable<IFeature> features, final SatInstance si) {
		...
		for (final IFeature feature : features) {
			...
			if ((parent != null)
				&& (!feature.getStructure().isMandatorySet()
					|| !parent.getStructure().isAnd())) {
				possibleFOFeatures.add(new int[] {
					-si.getVariable(parent.getName()),
					si.getVariable(feature.getName()) });
			}
		}
		final List<int[]> solution3 =
			LongRunningWrapper.runMethod(new ImplicationAnalysis(si, possibleFOFeatures), monitor.subTask(0));
		monitor.checkCancel();
		falseOptionalFeatures.clear();
		for (final int[] pair : solution3) {
			monitor.checkCancel();
			final IFeature feature =
				fm.getFeature((CharSequence) si.getVariableObject(pair[1]));
	...
	private List<IFeature> checkFeatureFalseOptional2(final BasicSolver solver, List<IFeature> foList) {
		...
		final List<int[]> solution3 =
			LongRunningWrapper.runMethod(new ImplicationAnalysis(solver, possibleFOFeatures));
		for (final int[] pair : solution3) {
	...
	private void checkFeatureHidden(final Iterable<IFeature> features) {
		...
		final Iterable<IFeature> hiddenFeatures =
			Functional.filter(features, new HiddenFeatureFilter());
		final List<String> hiddenLiterals =
			Functional.toList(Functional.map(hiddenFeatures, new Functional.IFunction<IFeature, String>() {
		...
		final int[] determinedHidden =
			LongRunningWrapper.runMethod(new IndeterminedAnalysis(si, hiddenLiterals));
	...
	private void checkValidity(final SatInstance si) {
		valid =
			LongRunningWrapper.runMethod(new ValidAnalysis(si)) != null;
	}
