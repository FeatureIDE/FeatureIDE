	...
	public IFeatureGraph analyze(IMonitor monitor) throws Exception {
		...
		if (model1 != null) {
			...
			outer: for (final Node clause : cnf.getChildren()) {
				...
				if (childrenCount == 2) {
					final int x =
						satInstance.getSignedVariable((Literal) literals[0]);
					final int y =
						satInstance.getSignedVariable((Literal) literals[1]);
					addRelation(x, y);
				} else {
					for (int i =
						0; i < (childrenCount
						...
						for (int j =
							i
								+ 1; j < childrenCount; j++) {
							final int y =
								satInstance.getSignedVariable((Literal) literals[j]);
							final int indexY =
								index[Math.abs(y)
									- 1];
			...
			visited =
				new byte[featureGraph.getSize()];
			complete =
				new boolean[featureGraph.getSize()];
		...
		if ((core[i] == 0)
			...
			inner1: for (int j =
				...
				if (AFeatureGraph.isWeakEdge(b)
					&& ((positive
						&& !(AFeatureGraph.isEdge(b, AFeatureGraph.EDGE_10Q)
							|| AFeatureGraph.isEdge(b, AFeatureGraph.EDGE_11Q)))
						|| (!positive
							&& !(AFeatureGraph.isEdge(b, AFeatureGraph.EDGE_00Q)
