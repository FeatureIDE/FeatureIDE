...
public class VisitorCheckFunctionCallNode implements Visitor{

	private boolean containFunctionCall = false;
	
	public VisitorCheckFunctionCallNode() {
		this.containFunctionCall = false;
	}
	
	public boolean containFunctionCall(){
		return this.containFunctionCall;
	}

	@Override
	public void run(Choice node) {
		for (int i = 0; i < node.getChildren().size(); i++){
			if (node.getChildren().get(i).getClass().getCanonicalName().equals("tree.FunctionCall")){
				this.containFunctionCall = true;
				break;
			} else {
