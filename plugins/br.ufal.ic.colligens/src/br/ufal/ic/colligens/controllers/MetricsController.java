...
public class MetricsController {
	...
	public void run() {
		...
		try {
			...
			int numberFiles =
				0;
			int numberFilesWithDirec =
				0;
			int directivesPerFile =
				0;
			int LinesOfCode =
				0;
			...
			Metrics statistics =
				new Metrics(NUMBER_OF_DIRECTIVES, ""
			...
			statistics =
				new Metrics(NUMBER_OF_FILES, ""
					+ numberFiles);
			list.add(statistics);

			statistics =
				new Metrics(NUMBER_OF_FILES_WITH_DIRECTIVES, ""
					+ numberFilesWithDirec);
			list.add(statistics);

			statistics =
				new Metrics("Directives per file (median)", ""
					+ (directivesPerFile));
			list.add(statistics);
