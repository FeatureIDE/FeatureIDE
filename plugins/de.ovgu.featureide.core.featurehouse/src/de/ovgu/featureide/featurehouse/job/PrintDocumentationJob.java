...
import de.ovgu.featureide.featurehouse.signature.documentation.DocumentationBuilder;
...
//				it.addFilter(new FeatureFilter(shortFeatureIDs));
		...
		DocumentationBuilder b = new DocumentationBuilder(featureProject);
		b.build(merger, null);
...
//		it.reset();
		...
		uztrew(folderPath, extFoldername, it);
		return true;
	}

	private void uztrew(final String folderPath, final String extFoldername, final SignatureIterator it) {
		IFolder folder;
		...
		for (AbstractClassFragment javaClass : structure.getClasses()) {
			...
			folder = CorePlugin.createFolder(project, path);
			IOConstants.writeToFile(
					folder.getFile(javaClass.getSignature().getName() + IOConstants.EXTENSION_JAVA),
					javaClass.toString());
			workMonitor.worked();
		}
		folder = CorePlugin.createFolder(project, arguments.foldername + "/doc/");
